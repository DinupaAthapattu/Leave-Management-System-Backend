...
export class LeavesController {
  ...
  async create(
    ...
    // Check if startDate and endDate are the same, then ensure dayPart is set to FULL_DAY if not provided
    if (start.getTime() === end.getTime() && (dayPart === 'FIRST_HALF' || dayPart === 'SECOND_HALF')) {
      // If the day is the same, dayPart should be set to FULL_DAY
      if (!dayPart) {
        throw new BadRequestException(
          'If startDate and endDate are the same, the dayPart must be FULL_DAY or specify FIRST_HALF/SECOND_HALF',
        );
      }
    } else if (start.getTime() !== end.getTime() && (dayPart === 'FIRST_HALF' || dayPart === 'SECOND_HALF')) {
      throw new BadRequestException(
        'Half day options (FIRST_HALF/SECOND_HALF) can only be used if the startDate and endDate are the same',
      );
    }
